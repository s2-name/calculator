# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'interface.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_Window(object):
    def setupUi(self, Window):
        Window.setObjectName("Window")
        Window.resize(368, 326)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("calculator_14445.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Window.setWindowIcon(icon)
        self.centralwidget = QtWidgets.QWidget(Window)
        # self.centralwidget.setMaximumSize(QtCore.QSize(508, 498))
        self.centralwidget.setObjectName("centralwidget")
        self.verticalLayout = QtWidgets.QVBoxLayout(self.centralwidget)
        self.verticalLayout.setObjectName("verticalLayout")
        self.display = QtWidgets.QLabel(self.centralwidget)
        font = QtGui.QFont()
        font.setPointSize(55)
        self.display.setFont(font)
        self.display.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.display.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTrailing|QtCore.Qt.AlignVCenter)
        self.display.setObjectName("display")
        self.verticalLayout.addWidget(self.display)
        self.keyboard = QtWidgets.QGridLayout()
        self.keyboard.setObjectName("keyboard")
        self.button_degree = QtWidgets.QPushButton(self.centralwidget)
        self.button_degree.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_degree.setObjectName("button_degree")
        self.keyboard.addWidget(self.button_degree, 1, 0, 1, 1)
        self.button_root = QtWidgets.QPushButton(self.centralwidget)
        self.button_root.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_root.setObjectName("button_root")
        self.keyboard.addWidget(self.button_root, 1, 1, 1, 1)
        self.button_back = QtWidgets.QPushButton(self.centralwidget)
        self.button_back.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_back.setText("")
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("delete.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.button_back.setIcon(icon1)
        self.button_back.setCheckable(False)
        self.button_back.setObjectName("button_back")
        self.keyboard.addWidget(self.button_back, 1, 2, 1, 1)
        self.button_div = QtWidgets.QPushButton(self.centralwidget)
        self.button_div.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_div.setObjectName("button_div")
        self.keyboard.addWidget(self.button_div, 1, 3, 1, 1)
        self.button_1 = QtWidgets.QPushButton(self.centralwidget)
        self.button_1.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_1.setObjectName("button_1")
        self.keyboard.addWidget(self.button_1, 2, 0, 1, 1)
        self.button_2 = QtWidgets.QPushButton(self.centralwidget)
        self.button_2.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_2.setObjectName("button_2")
        self.keyboard.addWidget(self.button_2, 2, 1, 1, 1)
        self.button_3 = QtWidgets.QPushButton(self.centralwidget)
        self.button_3.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_3.setObjectName("button_3")
        self.keyboard.addWidget(self.button_3, 2, 2, 1, 1)
        self.button_mul = QtWidgets.QPushButton(self.centralwidget)
        self.button_mul.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_mul.setObjectName("button_mul")
        self.keyboard.addWidget(self.button_mul, 2, 3, 1, 1)
        self.button_4 = QtWidgets.QPushButton(self.centralwidget)
        self.button_4.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_4.setObjectName("button_4")
        self.keyboard.addWidget(self.button_4, 3, 0, 1, 1)
        self.button_5 = QtWidgets.QPushButton(self.centralwidget)
        self.button_5.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_5.setObjectName("button_5")
        self.keyboard.addWidget(self.button_5, 3, 1, 1, 1)
        self.button_6 = QtWidgets.QPushButton(self.centralwidget)
        self.button_6.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_6.setObjectName("button_6")
        self.keyboard.addWidget(self.button_6, 3, 2, 1, 1)
        self.button_minus = QtWidgets.QPushButton(self.centralwidget)
        self.button_minus.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_minus.setObjectName("button_minus")
        self.keyboard.addWidget(self.button_minus, 3, 3, 1, 1)
        self.button_7 = QtWidgets.QPushButton(self.centralwidget)
        self.button_7.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_7.setObjectName("button_7")
        self.keyboard.addWidget(self.button_7, 4, 0, 1, 1)
        self.button_8 = QtWidgets.QPushButton(self.centralwidget)
        self.button_8.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_8.setObjectName("button_8")
        self.keyboard.addWidget(self.button_8, 4, 1, 1, 1)
        self.button_9 = QtWidgets.QPushButton(self.centralwidget)
        self.button_9.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_9.setObjectName("button_9")
        self.keyboard.addWidget(self.button_9, 4, 2, 1, 1)
        self.button_plus = QtWidgets.QPushButton(self.centralwidget)
        self.button_plus.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_plus.setObjectName("button_plus")
        self.keyboard.addWidget(self.button_plus, 4, 3, 1, 1)
        self.button_clear = QtWidgets.QPushButton(self.centralwidget)
        self.button_clear.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_clear.setObjectName("button_clear")
        self.keyboard.addWidget(self.button_clear, 5, 0, 1, 1)
        self.button_0 = QtWidgets.QPushButton(self.centralwidget)
        self.button_0.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_0.setObjectName("button_0")
        self.keyboard.addWidget(self.button_0, 5, 1, 1, 1)
        self.button_point = QtWidgets.QPushButton(self.centralwidget)
        self.button_point.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_point.setObjectName("button_point")
        self.keyboard.addWidget(self.button_point, 5, 2, 1, 1)
        self.button_eq = QtWidgets.QPushButton(self.centralwidget)
        self.button_eq.setFocusPolicy(QtCore.Qt.NoFocus)
        self.button_eq.setObjectName("button_eq")
        self.keyboard.addWidget(self.button_eq, 5, 3, 1, 1)
        self.verticalLayout.addLayout(self.keyboard)
        Window.setCentralWidget(self.centralwidget)

        self.retranslateUi(Window)
        QtCore.QMetaObject.connectSlotsByName(Window)

    def retranslateUi(self, Window):
        _translate = QtCore.QCoreApplication.translate
        Window.setWindowTitle(_translate("Window", "Calculator"))
        self.display.setText(_translate("Window", "0"))
        self.button_4.setText(_translate("Window", "4"))
        self.button_7.setText(_translate("Window", "7"))
        self.button_2.setText(_translate("Window", "2"))
        self.button_3.setText(_translate("Window", "3"))
        self.button_5.setText(_translate("Window", "5"))
        self.button_9.setText(_translate("Window", "9"))
        self.button_8.setText(_translate("Window", "8"))
        self.button_1.setText(_translate("Window", "1"))
        self.button_0.setText(_translate("Window", "0"))
        self.button_6.setText(_translate("Window", "6"))
        self.button_plus.setText(_translate("Window", "+"))
        self.button_minus.setText(_translate("Window", "-"))
        self.button_mul.setText(_translate("Window", "*"))
        self.button_div.setText(_translate("Window", "/"))
        self.button_eq.setText(_translate("Window", "="))
        self.button_clear.setText(_translate("Window", "C"))
        self.button_back.setText(_translate("Window", ""))
        self.button_point.setText(_translate("Window", "."))
        self.button_degree.setText(_translate("Window", "^"))
        self.button_root.setText(_translate("Window", "√"))
